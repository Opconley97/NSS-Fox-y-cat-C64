...
const getTheCat = async () => {
    const response = await fetch(`https://shibe.online/api/cats?count=1&urls=true&httpsUrls=false`) 
...
/*fetch(`https://shibe.online/api/cats?count=1&urls=true&httpsUrls=false`)
    .then(response => response.json())
    .then (
        (convertedData) => {
            const catImgElement = document.querySelector("#cat")
